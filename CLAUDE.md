...
Monolithic Go app (all `package main`), 8 source files + 1 test file + 1 embedded SPA template:

- **main.go** — Entry point. Token is optional — if provided, registers CLI bot; otherwise uses bots from DB. Starts ProxyManager for all bots, BridgeManager for protocol bridges, launches HTTP server.
...
- **bridge.go** — `BridgeManager` for multi-protocol bridges (Discord, Meshtastic, generic webhook). Translates external messages into Telegram Update JSON format and injects them into `processUpdate()`. Intercepts outgoing bot messages via `onMessageSent` hook and POSTs them to bridge callback URL. Maintains chat/message mappings for threading. Synthetic chat/user IDs avoid collision with real Telegram IDs.
